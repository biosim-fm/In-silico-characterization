...
def main(input_file):
    ...
    n_total = len(df)
    n_positive = np.sum(df['Name'].str.contains('Actives'))
    
    # True labels for ROC calculation: 1 for Actives and 0 for Decoys
    df['True Label'] = df['Name'].apply(lambda x: 1 if 'Actives' in x else 0)
    true_labels = df['True Label']
    scores = df['Energy']

    # Sort by scores in ascending order since lower scores are better
    df_sorted = df.sort_values(by='Energy', ascending=True)
    sorted_labels = df_sorted['True Label'].values
    ...
    ef_1 = calculate_ef(sorted_labels, n_total, n_positive, 0.01)
    ef_20 = calculate_ef(sorted_labels, n_total, n_positive, 0.20)
...
# Input file name
input_file = 'docking_summary.txt'  # Replace 'docking_summary.txt' with your actual file name
main(input_file)
